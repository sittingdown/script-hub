...
class Plugin(PluginBase):
    ...
    def _build_dashboard(self):
        ...
        lay, catches_lbl  = stat_pair("Session Catches");  sr.addLayout(lay); sr.addSpacing(36)
        lay, rate_lbl     = stat_pair("Catches / hr");     sr.addLayout(lay); sr.addSpacing(36)
        lay, runtime_lbl  = stat_pair("Runtime");          sr.addLayout(lay); sr.addSpacing(36)
        lay, lifetime_lbl = stat_pair("Lifetime Catches"); sr.addLayout(lay)
        sr.addStretch()
        root.addLayout(sr)
        root.addSpacing(12)
        root.addWidget(sep())
    ...
    def _build_settings(self):
        ...
        root.addSpacing(6)
        root.addWidget(lbl("Colors the bot clicks on. Use the overlay to pick from the game.",
                           sz=10, col=T3))
        root.addSpacing(8)
        ...
        root.addLayout(_hrow(lbl("Tolerance  (1–100)", sz=12, col=T2), tol))
        root.addSpacing(20)
        ...
        timing_rows = [
            ...
            ("Min Cluster Pixels",   "min_cluster_pixels", td.get("min_cluster_pixels", 30),   1,    1,    500),
        ]
